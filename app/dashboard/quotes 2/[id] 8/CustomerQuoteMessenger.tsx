...
interface CustomerQuoteMessengerProps {
  quoteId: string;
  repId: string;
  repName: string;
...
export function CustomerQuoteMessenger({ 
  quoteId, 
  repId, 
  repName,
  repAvatar,
  currentUserId
}: CustomerQuoteMessengerProps) {
  ...
  const fetchMessages = async () => {
    try {
      const response = await fetch(
        `/api/messages?recipient_id=${repId}&quote_id=${quoteId}`
      );
      if (response.ok) {
  ...
  return (
    <div className="flex h-[400px] flex-col rounded-xl border border-stone-200 bg-white shadow-sm">
      ...
      <div className="border-b border-stone-100 px-4 py-3">
        <h3 className="text-sm font-semibold text-navy-800">Messages</h3>
        <p className="text-xs text-slate-500">Chat about this quote</p>
      </div>
        ...
        {isLoading ? (
          ...
          <div className="flex h-full flex-col items-center justify-center text-center">
            ...
            <p className="text-xs text-slate-400">
              Ask {repName.split(' ')[0]} a question
            </p>
          </div>
        ) : (
