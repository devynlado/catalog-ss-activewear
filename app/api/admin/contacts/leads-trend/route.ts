...
function normalizeSource(value: string | null): TrackedSource | 'Other' {
  // NULL visitor_source = pre-tracking ("Untracked") entries.
...
/**
 * GET /api/admin/contacts/leads-trend?date_from=YYYY-MM-DD&date_to=YYYY-MM-DD
 * Returns per-day leads count bucketed by visitor source channel.
 */
export async function GET(request: NextRequest) {
  ...
  // miss leads whose UTC date differs from their tz date (timezone offset can be ±14h).
  ...
  if (error) {
    console.error('[Admin Contacts] Leads-trend query error:', error);
    return NextResponse.json({ error: 'Failed to fetch leads trend' }, { status: 500 });
  }
