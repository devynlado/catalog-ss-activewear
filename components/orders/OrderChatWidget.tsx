...
import {
  MessageCircle, X, Send, ImageIcon, Loader2, ChevronDown,
} from 'lucide-react';
...
export function OrderChatWidget({ orderNumber, customerName, customerEmail }: OrderChatWidgetProps) {
  ...
  const fetchMessages = useCallback(async () => {
    setIsLoading(true);
    try {
      const res = await fetch(`/api/orders/${orderNumber}/chat`);
      if (res.ok) {
  ...
  const uploadAttachment = async (file: File): Promise<string | null> => {
    try {
      const res = await fetch(`/api/orders/${orderNumber}/chat/upload`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  return (
    <>
      ...
      {/* Chat panel */}
      {isOpen && (
        <div className="fixed bottom-6 right-6 z-50 flex h-[500px] w-[380px] max-w-[calc(100vw-2rem)] flex-col overflow-hidden rounded-2xl border border-stone-200 bg-white shadow-2xl sm:max-h-[600px]">
          {/* Header */}
          <div className="flex items-center justify-between bg-gradient-to-r from-brand-600 to-brand-700 px-4 py-3">
            <div>
              ...
              <p className="text-xs text-brand-100">Order {orderNumber}</p>
            </div>
          ...
          <div className="flex-1 overflow-y-auto px-4 py-3">
            {isLoading ? (
              ...
              <div className="flex h-full flex-col items-center justify-center text-center px-4">
                ...
                <p className="text-sm font-medium text-slate-700">Need help with your order?</p>
